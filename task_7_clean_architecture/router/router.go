...
import (
	...
	"task_7_clean_architecture/data"
...
func StartEngine(port_number string) {
	...
	singleUseCase := useCase.NewUseCase( // DataBase is opened automatically while initializing here
		data.NewUserDataBase(),
		data.NewTaskDataBaseService(),
		infrastructure.NewJWTAuth(),
		infrastructure.NewPasswordService(),
	)
	log.Println("before Init")

	userC := controllers.NewUserController(singleUseCase)
	userAuth := middleware.NewUserAuth(singleUseCase)

	StartPublicRouter(router, userC)
	StartProtectedRouter(router, userAuth, userC)
	...
	singleUseCase.CloseDBConnection()
	fmt.Println()
